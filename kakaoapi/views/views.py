...
def ai_feedback_view(request):
    try:
        user = request.user
        df = load_runhistory_dataframe(user)

        if df.empty:
            return Response({
                "user": user.email,
                "ai_feedback": "최근 러닝 기록이 없어 AI 피드백을 제공할 수 없습니다."
            }, status=200)

        # 핵심 예측 호출
        feedback = predict_run_recommendation(df, user)
        ...
        print("🔥 예외 발생:", type(e).__name__, str(e))
        traceback.print_exc()
        return Response({
